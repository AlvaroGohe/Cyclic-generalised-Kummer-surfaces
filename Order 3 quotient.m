function ADEtype(pt)
_,F := IsHypersurfaceSingularity(pt,4);
...
X := Scheme(A6,[27*y1^2 - 81*y2 - 18*y2^2 - y2^3 + 54*y1*y3 + 27*y3^2, 3 + 3*x3 - y2, 243*x2^3 + 27*x2^3*y2 + 72*y2^3 + 8*y2^4 - 216*y1*y2*y3 - 216*y2*y3^2, 
 ...
 pi3 := map<Kum3->P3|[((-8*k1)/3 + (19*k2)/9 - 4*k4)^2/16, -1/8*((2*k1 - k2 + 2*k3)*((-8*k1)/3 + (19*k2)/9 - 4*k4)), (2*k1 - k2 + 2*k3)^2/4,  k1^2 - k1*k2 + k2^2 - k1*k3 - k2*k3 + k3^2]>;
